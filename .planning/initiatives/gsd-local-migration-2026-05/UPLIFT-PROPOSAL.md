...
| Reviewed Claude Design fit | Claude Design is a research-preview Anthropic Labs tool for conversational design/prototype creation with canvas iteration, screenshots/codebase context, chat/comments, exports, sharing, and Claude Code handoff. Known limitations include disappearing inline comments, compact-view save errors, large-codebase lag, and chat upstream errors. | https://support.claude.com/en/articles/14604416-get-started-with-claude-design |
...
| 3. Templates | CONTEXT template captures phase boundary, decisions, canonical refs `get-shit-done/templates/context.md:23-73`; PLAN template has must_haves but no doctrine fields `get-shit-done/templates/phase-prompt.md:15-31`; verification template has truth/artifact/wiring tables `get-shit-done/templates/verification-report.md:23-60`; CLAUDE template is generated from PROJECT/STACK/CONVENTIONS/ARCHITECTURE sections `get-shit-done/templates/claude-md.md:1-8`. | Touch CONTEXT, PLAN, planner-subagent, SUMMARY, VERIFICATION, CLAUDE template docs, and add a small DESIGN-BRIEF template for reviewed external prototypes. |
...
| 10. Docs | Upstream docs include command/config/user-guide surfaces; artifact and agent references are source-like docs. | Touch configuration/user guide/references only enough to document optional doctrine files, optional Claude Design/design-prototype input, and the no-hook/no-command stance. |
...
- **Current behavior with source citations:** Plan-phase can already pass a `UI_SPEC_PATH` and `SKETCH_FINDINGS_PATH` to the planner when those artifacts exist `get-shit-done/workflows/plan-phase.md:839-841`, but upstream does not define how external design prototypes become durable phase inputs. Claude Design supports conversational canvas generation, adding screenshots/codebase context, chat and inline comment iteration, exports, sharing, and Claude Code handoff; its guide also warns about intermittent comment persistence and large-codebase lag. Source: https://support.claude.com/en/articles/14604416-get-started-with-claude-design.
- **Proposed behavior:** Define an optional design-prototype intake lane for UI-heavy or visualization-heavy phases. The lane should let a user create Claude Design explorations, compare 2-3 directions, export or summarize the accepted direction, and store a reviewed `DESIGN-BRIEF.md` plus exported assets under a phase-specific `.planning/designs/` directory. The accepted brief can then be cited from CONTEXT canonical refs and consumed by planner as UI spec or sketch findings. Raw Claude Design output, inline comments, or Claude Code handoff code are not authoritative until summarized into the reviewed brief.
...
- **Patch sketch:** Add `get-shit-done/templates/design-brief.md`; update discuss/plan docs to recognize reviewed design briefs as canonical refs; add a reference note that Claude Design should link specific subdirectories rather than whole monorepos and that disappearing inline comments must be pasted into the chat or brief; update planner prompt to treat Claude Design handoff code as reference-only unless a PLAN explicitly scopes production adoption.
...
- **Risk:** Medium. Prototype output can seduce planning into implementing polish or future scope prematurely. Mitigate by requiring accepted design decisions to pass through CONTEXT, phase boundary, accessibility, responsiveness, and verification gates.
- **Testability:** Template test for DESIGN-BRIEF; workflow test that a design brief appears in canonical refs; planner prompt test that handoff code is reference-only; negative test that raw design exports do not become executable PLAN authority.
- **Rollback strategy:** Remove the optional design-brief template and docs. Existing phase CONTEXT/PLAN behavior remains unchanged.
...
- Design-intake tests: accepted DESIGN-BRIEF artifacts can be cited from CONTEXT and passed to planner; raw Claude Design exports or handoff code cannot bypass CONTEXT/PLAN authority.
...
- Optional design-brief intake for external prototypes, including Claude Design, as reviewed phase input.
