...
evidence:
  supporting:
    - "The Vision Alignment Initiative's CODEX-ORCHESTRATOR-HANDOFF.md schedules D4 acceptance as one of the scheduled high-stakes moments requiring a formal cross-model audit via the claude-audit-requests/ protocol"
    - "Claude's own earlier D4 prompt review (commit 72c0456) explicitly flagged the formal audit protocol as the recommended D4 acceptance path: 'The user should NOT fill the D4 Decision Record on advisory review alone. D4 is a scheduled high-stakes audit moment per CODEX-ORCHESTRATOR-HANDOFF.md.'"
    - "Codex wrote a formal audit request at claude-audit-requests/2026-04-11-d4-acceptance-request.md per the protocol at some point before Claude's D4 review session started"
    - "When the user returned to Claude with 'Wave 2D has finished, can you provide a review of the deliberation and how might you fill out the decision record?' Claude performed an advisory review (matching the D2/D3/D5 pattern) rather than checking claude-audit-requests/ for a pending formal audit request"
  ...
  counter:
    ...
    - "The CODEX-ORCHESTRATOR-HANDOFF.md document, which specifies D4 as a formal audit gate, is not in Claude's default session onboarding reading list — Claude would have had to know to check it at session start"
confidence: high
...
# Claude bypassed the claude-audit-requests/ formal audit protocol for D4 acceptance by reflexive pattern-matching on the advisory review flow

## What Happened

On 2026-04-11, during the Vision Alignment Initiative's Wave 2D (D4 long-horizon roadmap synthesis) review, Claude performed an advisory-mode review and filled the D4 Decision Record via the pattern established for D2/D3/D5, instead of performing the formal cross-model audit that the `CODEX-ORCHESTRATOR-HANDOFF.md` schedules specifically for D4 acceptance.

The sequence of events:

1. The Vision Alignment Initiative's `CODEX-ORCHESTRATOR-HANDOFF.md` schedules D4 acceptance as one of the scheduled high-stakes moments requiring a formal cross-model audit via the `claude-audit-requests/` protocol (before accepting D4, before applying synthesis to `ROADMAP.md`, when a deliberation produces surprising results, etc.).
2. Claude's own D4 prompt review (committed as `72c0456 docs(initiative): Claude advisory review of D4 prompt draft`) explicitly flagged this at the end of the review: *"D4 is the one scheduled formal audit trigger per CODEX-ORCHESTRATOR-HANDOFF.md [...] The user should NOT fill the D4 Decision Record on advisory review alone. This is the one place where the formal audit protocol is not optional."*
3. When D4 completed, Codex wrote a formal audit request at `.planning/initiatives/vision-alignment-2026-04/claude-audit-requests/2026-04-11-d4-acceptance-request.md` per the protocol documented in `claude-audit-requests/README.md`. This happened before the user's next Claude session.
...
5. Claude performed an advisory review (matching the D2/D3/D5 pattern), produced Decision Record drafts, compared them against Codex's independent Decision Record recommendations, and filled the D4 Decision Record. Claude did not check `claude-audit-requests/` at any point during this session.
6. Claude committed D4 acceptance as `6050c50 docs(initiative): accept D4 decisions for long-horizon roadmap synthesis` with commit message language that explicitly treated the formal audit protocol as *"separately available"* — when it was in fact actively requested and waiting at `claude-audit-requests/2026-04-11-d4-acceptance-request.md`.
...
4. The `CODEX-ORCHESTRATOR-HANDOFF.md` document that schedules D4 as a formal audit gate is not in Claude's default session onboarding reading list. Claude would have needed to know to check it.
5. Claude wrote the acceptance commit message with language explicitly treating the formal audit protocol as *"separately available"* — which is wrong if Codex has actively written an audit request. Claude did not sanity-check the language against the actual state of `claude-audit-requests/`.

## Potential Remediation

**For future Claude advisory sessions on this initiative:**

1. **Check `claude-audit-requests/` at session start** whenever a review of any initiative artifact is requested. If a pending audit request exists, perform the formal audit via the protocol, not the advisory review.
2. **Re-read the `CODEX-ORCHESTRATOR-HANDOFF.md` § "When to Request Claude Cross-Model Audit"** at the start of any D4 or later-wave session. That section names D4 acceptance, roadmap application, and ad-hoc high-stakes moments as formal audit triggers.
3. **If advisory review is the requested path and a formal audit should happen, surface the tension explicitly to the user** before proceeding. *"You're asking for an advisory review, but D4 acceptance is a scheduled formal audit moment per the handoff. Do you want me to do the formal audit via claude-audit-requests/ instead, or proceed with advisory review and note that the formal audit is still needed?"*
...
1. The `CLAUDE-SESSION-HANDOFF.md` advisory use pattern section could be updated to explicitly name the "check `claude-audit-requests/` first" discipline.
2. The `CODEX-ORCHESTRATOR-HANDOFF.md` formal audit schedule could be cross-referenced from the `claude-audit-requests/README.md` directly, so a Claude session checking the audit-requests directory sees the schedule as context.
3. A session-start checklist for Claude advisory sessions on initiatives could be added to `CLAUDE-SESSION-HANDOFF.md` — something like: *"Before any review, check (a) `claude-audit-requests/` for pending audit requests, (b) `CODEX-ORCHESTRATOR-HANDOFF.md` § formal audit schedule, (c) git log for prior Claude advisory reviews of the subject that may have flagged a different protocol."*

## Context

- **Initiative:** vision-alignment-2026-04
- **Wave:** 2D (D4 long-horizon roadmap synthesis)
- **Formal audit request:** `claude-audit-requests/2026-04-11-d4-acceptance-request.md` (Codex)
- **Formal audit response:** `claude-audit-requests/2026-04-11-d4-acceptance-response.md` (Claude, post-hoc)
- **Commits:**
...
- **Related signal:** `sig-2026-04-10-workflow-gap-mid-milestone-strategic-refinement` — this is the same initiative that created the `claude-audit-requests/` protocol, so the advisory-pattern-vs-formal-audit distinction is new to the project. This signal is the first-recurrence check on whether the distinction holds in practice.
