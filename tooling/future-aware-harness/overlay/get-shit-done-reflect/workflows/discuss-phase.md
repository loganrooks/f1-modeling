...
2. **gsdr-planner** — Reads CONTEXT.md to know WHAT decisions are locked
   ...
   - "Future Awareness: preserve transport-neutral execution boundary" → planner carries it into `future_preservation` rather than silently collapsing the seam
...
Only include sections that have content. Reference: `@get-shit-done-reflect/references/claim-types.md` for type vocabulary.
</context_model>
...
Enable text mode:
- Per-session: pass `--text` flag to any command (e.g., `\$gsdr-discuss-phase --text`)
...
```bash
INIT=$(node "$HOME/.codex/get-shit-done-reflect/bin/gsd-tools.cjs" init phase-op "${PHASE}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_ADVISOR=$(node "$HOME/.codex/get-shit-done-reflect/bin/gsd-tools.cjs" agent-skills gsdr-advisor 2>/dev/null)
```
...
Use \$gsdr-progress ${GSD_WS} to see available phases.
```
Exit workflow.
...
```bash
DISCUSS_MODE=$(node "$HOME/.codex/get-shit-done-reflect/bin/gsd-tools.cjs" config-get workflow.discuss_mode 2>/dev/null || echo "exploratory")
```
...
- options:
  - "Continue and replan after" — Capture context, then run \$gsdr-plan-phase {X} ${GSD_WS} to replan
...
```bash
TODO_MATCHES=$(node "$HOME/.codex/get-shit-done-reflect/bin/gsd-tools.cjs" todo match-phase "${PHASE_NUMBER}")
```
...
1b. **Initialize canonical refs accumulator** — Start building the `<canonical_refs>` list for CONTEXT.md. This accumulates throughout the entire discussion, not just this step.
   ...
   **Source 2b (now):** If `.planning/LONG-ARC.md` exists and materially constrains the phase, add it as a doctrine reference rather than leaving it ambient. In this harness, treat `.planning/LONG-ARC.md` as the default canonical doctrine ref unless the phase truly does not touch long-arc posture.
...
1c. **Derive future awareness** — Build an internal `<future_awareness>` accumulator for CONTEXT.md using ROADMAP.md, PROJECT.md, REQUIREMENTS.md, `.planning/LONG-ARC.md`, prior decisions, and relevant initiative/review artifacts cited by the phase.
   ...
   - **Current Posture** — the project's present trust, visibility, service-obligation, or architecture stance that downstream work must respect
   ...
   Future awareness is not a place to smuggle future scope into the current phase. It is a steering constraint on HOW current work should be shaped so later phases remain possible.
...
1. Check for USER-PROFILE.md:
   ```bash
   PROFILE_PATH="$HOME/.codex/get-shit-done-reflect/USER-PROFILE.md"
   ```
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(node "$HOME/.codex/get-shit-done-reflect/bin/gsd-tools.cjs" resolve-model gsdr-advisor-researcher --raw)
   ```
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @$HOME/.codex/agents/gsdr-advisor-researcher.md for your role and instructions.
...
Every entry needs a full relative path — not just a name. Seed `.planning/LONG-ARC.md`
by default when it exists unless the phase truly does not touch long-arc posture.]
...
### Current Posture
[The current trust, visibility, service-obligation, or architecture stance in project terms. Prefer concrete framing over vague aspiration.]
...
[Do not add tech-debt disposition or research-disposition bookkeeping here. This section captures steering constraints only.]
...
**Also available:**
- `\$gsdr-discuss-phase ${PHASE} --chain ${GSD_WS}` — re-run with auto plan+execute after
- `\$gsdr-plan-phase ${PHASE} --skip-research ${GSD_WS}` — plan without research
- `\$gsdr-ui-phase ${PHASE} ${GSD_WS}` — generate UI design contract before planning (if phase has frontend work)
...
if [ "$commit_docs" = "true" ]; then
  node "$HOME/.codex/get-shit-done-reflect/bin/gsd-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
fi
```
...
- **Reference doc:** `get-shit-done-reflect/references/claim-types.md`

```
Task(agent="gsdr-context-checker", input="Verify claim integrity for Phase ${PHASE}. CONTEXT.md: ${phase_dir}/${padded_phase}-CONTEXT.md, DISCUSSION-LOG.md: ${phase_dir}/${padded_phase}-DISCUSSION-LOG.md, Reference: get-shit-done-reflect/references/claim-types.md")
```
...
```bash
if [ "$commit_docs" = "true" ]; then
  node "$HOME/.codex/get-shit-done-reflect/bin/gsd-tools.cjs" commit "docs(${padded_phase}): context-checker verification" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
fi
```
</step>
...
```bash
node "$HOME/.codex/get-shit-done-reflect/bin/gsd-tools.cjs" state record-session \
...
```bash
node "$HOME/.codex/get-shit-done-reflect/bin/gsd-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
2. **Sync chain flag with intent** — if user invoked manually (no `--auto` and no `--chain`), clear the ephemeral chain flag from any previous interrupted `--auto` chain. This does NOT touch `workflow.auto_advance` (the user's persistent settings preference):
   ...
   if [[ ! "{{GSD_ARGS}}" =~ --auto ]] && [[ ! "{{GSD_ARGS}}" =~ --chain ]]; then
     node "$HOME/.codex/get-shit-done-reflect/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false 2>/dev/null
   fi
   ```
3. Read both the chain flag and user preference:
   ```bash
   AUTO_CHAIN=$(node "$HOME/.codex/get-shit-done-reflect/bin/gsd-tools.cjs" config-get workflow._auto_chain_active 2>/dev/null || echo "false")
   AUTO_CFG=$(node "$HOME/.codex/get-shit-done-reflect/bin/gsd-tools.cjs" config-get workflow.auto_advance 2>/dev/null || echo "false")
   ```
...
```bash
node "$HOME/.codex/get-shit-done-reflect/bin/gsd-tools.cjs" config-set workflow._auto_chain_active true
```
...
- **PHASE COMPLETE** → Full chain succeeded. Display:
  ...
  Next: \$gsdr-discuss-phase ${NEXT_PHASE} ${WAS_CHAIN ? "--chain" : "--auto"} ${GSD_WS}
  <sub>/clear first → fresh context window</sub>
  ```
- **PLANNING COMPLETE** → Planning done, execution didn't complete:
  ```
  Auto-advance partial: Planning complete, execution did not finish.
  Continue: \$gsdr-execute-phase ${PHASE} ${GSD_WS}
  ```
- **PLANNING INCONCLUSIVE / CHECKPOINT** → Stop chain:
  ```
  Auto-advance stopped: Planning needs input.
  Continue: \$gsdr-plan-phase ${PHASE} ${GSD_WS}
  ```
- **GAPS FOUND** → Stop chain:
  ```
  Auto-advance stopped: Gaps found during execution.
  Continue: \$gsdr-plan-phase ${PHASE} --gaps ${GSD_WS}
  ```
