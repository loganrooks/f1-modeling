...
- `gsdr-phase-researcher` — Reads decisions to focus research (e.g., "card layout" → research card component patterns)
- `gsdr-planner` — Reads decisions to create specific tasks (e.g., "infinite scroll" → task includes virtualization)
...
[List every spec, ADR, initiative review, or doctrine file that defines requirements or constraints for this phase. Use full relative paths so downstream agents can read them directly.]
[Seed `.planning/LONG-ARC.md` by default when it exists unless the phase truly does not touch long-arc posture.]

### [Topic area 1]
- `path/to/spec-or-adr.md` — [What this doc decides or defines]
- `path/to/doc.md` §N — [Specific section and why it matters]

### [Topic area 2]
- `path/to/decision-anchor.md` — [What downstream constraint it carries]

[If no external specs: "No external specs — requirements are fully captured in the steering sections above"]
...
### Current Posture
[The project's current trust, visibility, service-obligation, or architecture stance that downstream work must respect.]

### Future Shape Notes
[Constrained notes about later wrapper, sibling-surface, or future-shape evolution that should influence current seams without pulling future scope forward.]
...
**Note:** The template above shows the discuss-mode variant. In exploratory mode (`workflow.discuss_mode = exploratory`), the `<decisions>` section is replaced with: `<working_model>`, `<constraints>`, `<guardrails>`, `<questions>`, `<dependencies>`. `Canonical References`, `Future Awareness`, `Code Context`, `Specific Ideas`, and `Deferred Ideas` still remain part of the steering brief. Claims use `[type:verification]` notation per `references/claim-types.md`.

<good_examples>

Examples below illustrate tone and specificity. They may omit some newer required steering sections for brevity. In the active harness, always include `Canonical References` and `Future Awareness`.
...
- `gsdr-phase-researcher` uses decisions to focus investigation
- `gsdr-planner` uses decisions + research to create executable tasks
