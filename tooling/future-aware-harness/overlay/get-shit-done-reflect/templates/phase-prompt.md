...
> **Note:** Planning methodology is in `agents/gsdr-planner.md`.
...
planning_posture:
  context_mode: full_context    # full_context | legacy_context | reduced_guarantee_no_context
  context_path: null            # Path to CONTEXT.md when present
  canonical_refs_resolved: false

future_preservation:
  - source_bucket: Protected Seams
    item: "Preserve a material future-aware item from CONTEXT.md"
    disposition: preserved_seam   # preserved_seam | sequencing_choice | validation_task | non_action_rationale
    plan_consequence: "Explain the concrete planning consequence"

tech_debt_disposition:
  - id: VA-TD-00
    status: intentionally_deferred  # trigger_fired | retired | intentionally_deferred | not_applicable
    note: "Explain what this plan does with the debt trigger"
...
<execution_context>
@$HOME/.codex/get-shit-done-reflect/workflows/execute-plan.md
@$HOME/.codex/get-shit-done-reflect/templates/summary-standard.md
[If plan contains checkpoint tasks (type="checkpoint:*"), add:]
@$HOME/.codex/get-shit-done-reflect/references/checkpoints.md
</execution_context>
...
[If a CONTEXT.md exists for this phase, reference it directly]
...
<!-- For checkpoint task examples and patterns, see @$HOME/.codex/get-shit-done-reflect/references/checkpoints.md -->
...
| `planning_posture` | Yes | Records whether planning used full context, legacy context, or explicit reduced-guarantee bypass. |
| `future_preservation` | Yes when future-awareness is material | Structured mapping of future-aware items to preserved seam, sequencing, validation, or explicit non-action. |
| `tech_debt_disposition` | Yes when debt ids are relevant | Structured disposition for touched rewrite triggers or accepted debt. |
| `must_haves` | Yes | Goal-backward verification criteria (see below) |

**Wave is pre-computed:** Wave numbers are assigned during `\$gsdr-plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.

**Must-haves enable verification:** The `must_haves` field carries goal-backward requirements from planning to execution. After all plans complete, execute-phase spawns a verification subagent that checks these criteria against the actual codebase.

**Planning posture must stay honest:** `planning_posture.context_mode` must reflect the real planning input. Do not mark a plan as `full_context` if the run used `--allow-no-context` or only consumed a legacy steering brief.

**Future preservation must be auditable:** When `CONTEXT.md` includes material `Future Awareness`, each plan needs concrete `future_preservation` entries. Generic notes like "keep future options open" are not enough.

**Debt disposition is not optional bookkeeping:** If the phase touches a relevant `TECH-DEBT.md` trigger, the plan must say whether the trigger fired, was retired, remains intentionally deferred, or is genuinely not applicable.
...
See `$HOME/.codex/get-shit-done-reflect/references/tdd.md` for TDD plan structure.
...
See `$HOME/.codex/get-shit-done-reflect/templates/user-setup.md` for full schema and examples
...
See `$HOME/.codex/get-shit-done-reflect/workflows/verify-phase.md` for verification logic.
