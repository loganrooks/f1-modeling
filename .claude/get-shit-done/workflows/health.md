...
TASKS_DIR="/home/rookslog/workspace/projects/f1-modeling/.claude/worktrees/gsd-local-install-2026-05/.claude/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$( (find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null || true) | wc -l )
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in /home/rookslog/workspace/projects/f1-modeling/.claude/worktrees/gsd-local-install-2026-05/.claude/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf /home/rookslog/workspace/projects/f1-modeling/.claude/worktrees/gsd-local-install-2026-05/.claude/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
